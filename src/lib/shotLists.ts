// Latest change: added setShotAssignee so each shot can be assigned to a specific crew member.
import {
  collection,
  addDoc,
  updateDoc,
...
export async function setShotAssignee(itemId: string, assignedTo: string): Promise<void> {
  await updateDoc(doc(db, "shotLists", itemId), stripUndefined({ assignedTo: assignedTo || undefined }));
}
