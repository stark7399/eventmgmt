// Latest change: bigger, bolder treatment throughout - this screen gets used live at weddings, so checkboxes, filter chips, and buttons are now extra-large and high-contrast.
"use client";
...
export default function EventDayPage({
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="sticky -top-4 z-10 -mx-4 bg-stone-50 px-4 pb-4 pt-4 dark:bg-stone-950 md:-mx-6 md:px-6">
        <div className="mb-3 flex items-center justify-between">
          <Link
            href={`/projects/${id}`}
            className="flex items-center gap-1.5 text-base font-semibold text-stone-500 dark:text-stone-400"
          >
            <ArrowLeft size={20} />
            {project.clientName}
          </Link>
          {project.phase === "event-day" && (
            <button
              ...
              disabled={ending}
              className="rounded-xl bg-stone-800 px-4 py-2.5 text-sm font-bold text-white shadow-sm disabled:opacity-50 dark:bg-stone-700"
            >
        ...
        <div className="mb-1.5 flex items-center justify-between text-base">
          ...
          {skippedCount > 0 && (
            <span className="font-semibold text-amber-600 dark:text-amber-400">{skippedCount} skipped</span>
          )}
        </div>
        ...
        <div className="mt-3.5 flex gap-2 overflow-x-auto pb-1">
          <button
            ...
            className={`shrink-0 rounded-full px-4 py-2 text-sm font-bold ${
              activeCategory === "All"
                ...
                : "border-2 border-stone-200 bg-white text-stone-500 dark:border-stone-800 dark:bg-stone-900 dark:text-stone-400"
            }`}
          >
            All
          </button>
          {categories.map((cat) => {
            ...
            return (
              ...
              >
                {!active && <span className={`h-2 w-2 rounded-full ${color.dot}`} />}
                {cat}
              </button>
            );
          })}
      ...
      {totalCount === 0 ? (
        <p className="mt-6 rounded-2xl border-2 border-dashed border-stone-200 px-4 py-10 text-center text-sm text-stone-400 dark:border-stone-800 dark:text-stone-600">
...
function ShotRow({
  ...
  return (
    <li
      className={`rounded-2xl border-l-[6px] border-y-2 border-r-2 border-stone-200 bg-white px-4 py-4 shadow-sm dark:border-y-stone-800 dark:border-r-stone-800 dark:bg-stone-900 ${
    ...
    >
      <div className="flex items-center gap-3.5">
        <button
          ...
          className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border-[3px] ${
            item.done
        ...
        <div className="min-w-0 flex-1">
          ...
          {item.skipped && item.skipNote && (
            <p className="mt-0.5 truncate text-sm font-medium text-amber-600 dark:text-amber-400">
              Skipped: {item.skipNote}
            </p>
          )}
          {item.assignedTo && (
            <p className="mt-0.5 flex items-center gap-1.5 text-sm text-stone-400 dark:text-stone-500">
              <User size={14} />
              {item.assignedTo}
            </p>
          )}
...
function SkipNoteModal({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-sm rounded-t-3xl border-2 border-stone-200 bg-white p-5 dark:border-stone-800 dark:bg-stone-900 sm:rounded-3xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-bold text-stone-900 dark:text-stone-100">{item.label}</h3>
          <button
            onClick={onClose}
            className="rounded-lg p-1.5 text-stone-400 hover:bg-stone-100 dark:text-stone-500 dark:hover:bg-stone-800"
            aria-label="Close"
          >
            <X size={24} />
          </button>
        ...
        <textarea
          ...
          autoFocus
          className="mb-4 w-full rounded-xl border-2 border-stone-200 bg-stone-50 px-3.5 py-3 text-base text-stone-900 placeholder-stone-400 outline-none focus:border-amber-500 dark:border-stone-700 dark:bg-stone-950 dark:text-stone-100"
        />
        ...
        <div className="flex gap-2.5">
          {item.skipped && (
            <button
              onClick={handleClear}
              disabled={saving}
              className="flex-1 rounded-xl bg-stone-100 py-3.5 text-base font-bold text-stone-700 disabled:opacity-50 dark:bg-stone-800 dark:text-stone-100"
            >
              Clear Skip
            </button>
          )}
          <button
            onClick={handleSkip}
            disabled={saving}
            className="flex-1 rounded-xl bg-amber-500 py-3.5 text-base font-bold text-white disabled:opacity-50"
          >
