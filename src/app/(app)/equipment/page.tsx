// Latest change: bigger, bolder cards and header throughout; project picker now shows the full start-end date range.
"use client";
    ...
    <div className="mx-auto max-w-2xl">
      <div className="mb-5 flex items-center justify-between">
        <h1 className="flex items-center gap-3 text-3xl font-bold text-stone-900 dark:text-stone-100">
          <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-section-equipment/15 text-section-equipment">
            <Camera size={26} />
          </span>
          Equipment
        </h1>
        {canEdit && (
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl bg-section-equipment px-4 py-2.5 text-base font-semibold text-white shadow-sm"
          >
            <Plus size={20} />
            Add Gear
          </button>
      ...
      <div className="mb-5">
        ...
        <select
          ...
          className="w-full rounded-xl border-2 border-stone-200 bg-white px-4 py-3.5 text-base font-medium text-stone-900 outline-none focus:border-section-equipment dark:border-stone-800 dark:bg-stone-900 dark:text-stone-100"
        >
...
function ProjectGearView({
  ...
  return (
    <div className="flex flex-col gap-5">
      <section>
        <h2 className="mb-2.5 text-sm font-bold uppercase tracking-wide text-section-equipment">
                ...
                bookingAction={
                  canEdit ? (
                    <button
                      ...
                      className="rounded-xl bg-stone-100 px-4 py-2 text-sm font-bold text-stone-700 dark:bg-stone-800 dark:text-stone-200"
                    >
                      Unbook
                    </button>
                  ) : null
                  ...
                  bookingAction={
                    canEdit && !conflict ? (
                      <button
                        ...
                        className="rounded-xl bg-section-equipment px-4 py-2 text-sm font-bold text-white"
                      >
                        Book
                      </button>
                    ) : null
...
function BookedProjectCrew({ project }: { project: Project }) {
  ...
  return (
    <div className="text-sm">
      <p className="font-semibold text-stone-700 dark:text-stone-300">Booked for {project.clientName}</p>
      <p className="mt-1 flex items-center gap-1.5 text-stone-500 dark:text-stone-400">
        <Users size={15} className="shrink-0 text-section-equipment" />
        {crew === null
          ? "Loading crew..."
          : crew.length === 0
...
function GearRow({
  ...
  return (
    <li className="rounded-2xl border-2 border-stone-200 bg-white px-4 py-4 shadow-sm dark:border-stone-800 dark:bg-stone-900">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-section-equipment/15 text-section-equipment">
            <Camera size={20} />
          </span>
          <div>
            <p className="text-base font-bold text-stone-900 dark:text-stone-100">{item.name}</p>
        ...
        {canEdit && (
          <div className="flex shrink-0 items-center gap-1">
            <button
              onClick={() => onEdit(item)}
              className="rounded-lg p-2.5 text-stone-400 hover:bg-section-equipment/10 hover:text-section-equipment dark:text-stone-600"
      ...
      {conflictNote && (
        <div className="mt-3 flex items-center gap-1.5 border-t border-stone-100 pt-3 text-sm font-semibold text-red-500 dark:border-stone-800 dark:text-red-400">
      ...
      {bookingAction && (
        <div className="mt-3 flex justify-end border-t border-stone-100 pt-3 dark:border-stone-800">
          {bookingAction}
        </div>
      )}
  ...
  return (
    ...
    >
      ...
      <input
        ...
        className="rounded-xl border-2 border-stone-200 bg-stone-50 px-3 py-3 text-base text-stone-900 placeholder-stone-400 outline-none focus:border-section-equipment dark:border-stone-700 dark:bg-stone-950 dark:text-stone-100"
      />
      <div className="flex gap-2">
        <select
          value={category}
          onChange={(e) => setCategory(e.target.value as EquipmentItem["category"])}
          className="flex-1 rounded-xl border-2 border-stone-200 bg-stone-50 px-3 py-3 text-base text-stone-900 outline-none focus:border-section-equipment dark:border-stone-700 dark:bg-stone-950 dark:text-stone-100"
        >
          {CATEGORIES.map((c) => (
        ...
        <input
          ...
          className="flex-1 rounded-xl border-2 border-stone-200 bg-stone-50 px-3 py-3 text-base text-stone-900 placeholder-stone-400 outline-none focus:border-section-equipment dark:border-stone-700 dark:bg-stone-950 dark:text-stone-100"
        />
      </div>
      <button
        type="submit"
        disabled={saving}
        className="rounded-xl bg-section-equipment py-3.5 text-base font-semibold text-white disabled:opacity-50"
      >
