// Latest change: shows the currently live task, next scheduled task, and any live event, alongside the existing task/event counts - redesigned with theme-aware cards.
"use client";

import { useEffect, useMemo, useState } from "react";
import Link from "next/link";
import { format, isFuture, isToday } from "date-fns";
import { ListChecks, Calendar, CheckCircle2, Radio, ArrowRight } from "lucide-react";
import { useAuth } from "@/contexts/AuthContext";
import { subscribeToTasks } from "@/lib/tasks";
import { subscribeToEvents } from "@/lib/calendarEvents";
import { subscribeToProjects, findLiveProject } from "@/lib/projects";
...
export default function DashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      {liveProject && (
        <Link
          href={`/event-day/${liveProject.id}`}
          className="mb-4 flex items-center gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3.5 dark:border-red-900/40 dark:bg-red-950/30"
        >
