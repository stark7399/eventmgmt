// Latest change: merged with the old view-only Overview tab - event type, location, and start/end dates are now editable here too, not just creative brief/budget/contacts. Currency in ₹, 16px inputs to stop mobile auto-zoom.
"use client";
...
export default function IntakeForm({ project }: { project: Project }) {
  ...
  const [startDate, setStartDate] = useState(format(project.eventStartDate));
  const [endDate, setEndDate] = useState(format(project.eventEndDate));
  ...
  async function handleSave(e: FormEvent) {
    ...
    try {
      await updateProject(project.id, {
        eventType: eventType.trim() || project.eventType,
    ...
    <form onSubmit={handleSave} className="flex flex-col gap-6">
      <section className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-semibold text-stone-700 dark:text-stone-300">Event Type</label>
          <input
            type="text"
            value={eventType}
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-semibold text-stone-700 dark:text-stone-300">Location</label>
          <input
            type="text"
            value={location}
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-semibold text-stone-700 dark:text-stone-300">Event Start Date</label>
          <input
            type="date"
            value={startDate}
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-semibold text-stone-700 dark:text-stone-300">Event End Date</label>
          <input
            type="date"
            value={endDate}
      ...
      <section>
        ...
        <textarea
          ...
          className="w-full rounded-lg border border-stone-200 bg-white px-3 py-2.5 text-base text-stone-900 placeholder-stone-400 outline-none focus:border-section-projects dark:border-stone-700 dark:bg-stone-900 dark:text-stone-100"
        />
      </section>
      ...
      <section>
        ...
        <div className="flex flex-col gap-1.5 sm:flex-row">
          <input
            ...
            className="flex-1 rounded-lg border border-stone-200 bg-white px-3 py-2 text-base text-stone-900 placeholder-stone-400 outline-none focus:border-section-projects dark:border-stone-700 dark:bg-stone-900 dark:text-stone-100"
          />
          <input
            type="text"
          ...
          <input
            ...
            className="rounded-lg border border-stone-200 bg-white px-3 py-2 text-base text-stone-900 placeholder-stone-400 outline-none focus:border-section-projects dark:border-stone-700 dark:bg-stone-900 dark:text-stone-100 sm:w-32"
          />
          <button
            type="button"
            onClick={addContact}
      ...
      <div className="flex items-center gap-3">
        <button
          type="submit"
          disabled={saving}
          className="flex items-center gap-1.5 rounded-lg bg-section-projects px-5 py-2.5 font-semibold text-white disabled:opacity-50"
        >
          <Save size={16} />
