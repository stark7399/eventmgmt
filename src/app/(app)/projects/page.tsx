// Latest change: full redesign (theme-aware, indigo accent, cards) plus toast notification on project creation.
"use client";
    ...
    <div className="mx-auto max-w-2xl">
      <div className="mb-4 flex items-center justify-between">
        <h1 className="flex items-center gap-2 text-2xl font-bold text-stone-900 dark:text-stone-100">
          <FolderKanban className="text-section-projects" size={26} />
          Projects
        </h1>
        {user?.role === "admin" && (
          <button
          ...
          {projects.map((project) => (
            <li key={project.id}>
              <Link
                ...
                className="flex items-center justify-between gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3.5 shadow-sm dark:border-stone-800 dark:bg-stone-900"
              >
                ...
                <span
                  className={`shrink-0 rounded-full px-2.5 py-0.5 text-xs font-semibold ${PHASE_COLOR[project.phase]}`}
                >
  ...
  return (
    ...
    >
      <input
        ...
        className="rounded-lg border border-stone-200 bg-stone-50 px-3 py-2.5 text-stone-900 placeholder-stone-400 outline-none focus:border-section-projects dark:border-stone-700 dark:bg-stone-950 dark:text-stone-100"
      />
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 rounded-lg border border-stone-200 bg-stone-50 px-3 py-2.5 text-stone-900 placeholder-stone-400 outline-none focus:border-section-projects dark:border-stone-700 dark:bg-stone-950 dark:text-stone-100"
        />
        <input
          type="date"
          value={eventDate}
          onChange={(e) => setEventDate(e.target.value)}
          className="rounded-lg border border-stone-200 bg-stone-50 px-3 py-2.5 text-stone-900 outline-none focus:border-section-projects dark:border-stone-700 dark:bg-stone-950 dark:text-stone-100"
        />
      </div>
      <input
        type="text"
      ...
      <button
        type="submit"
        disabled={saving}
        className="rounded-lg bg-section-projects py-2.5 font-semibold text-white disabled:opacity-50"
      >
